...
public class DatasetController {
    ...
    private Map<String,Object> importOrders(MultipartFile file) {
        ...
        } catch(Exception e){ throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Could not parse orders CSV: "+e.getMessage(),e); }
    }
    private Map<String,Object> importPayments(MultipartFile file) {
        ...
        try (var reader=new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            updateBatch(batch,count,warnings); return result(batch,count,warnings);
        } catch(Exception e){ throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Could not parse payments CSV: "+e.getMessage(),e); }
    }
    ...
    private static OffsetDateTime date(String value,boolean payment){if(value==null||value.isBlank())return null; LocalDateTime parsed=payment?LocalDateTime.parse(value.trim(),PAYMENT_DATE):LocalDateTime.parse(value.trim().replace("Z","")); return parsed.atOffset(ZoneOffset.UTC);}
}